...
## 更新摘要
**变更内容**   
- 完全重新设计了热信号检测系统，采用分层决策树替代全局阈值方法
- 新增三种状态层级：local_fire_visible、thermal_sensor_signal和has_heat_signal
- 更新了传感器可见性检测机制，支持更精确的热信号分类
- 改进了综合判断逻辑，增强了误报过滤能力
...
## 引言
本技术文档围绕"热信号分层判定系统"展开，聚焦于传感器可见性检测、热信号阈值策略、综合判断逻辑与误报过滤机制，并结合代码实现解释其在火灾前沿识别中的准确性与鲁棒性提升。**最新更新**：系统已完全重新设计，采用分层决策树替代全局阈值方法，支持local_fire_visible、thermal_sensor_signal和has_heat_signal三种状态层级，显著提高了在噪声环境下的稳定性与可解释性。
...
- [rl_environment_baseline.py:565-611](file://environment_variables/environment_variables/rl_environment_baseline.py#L565-611)

### 热信号分层决策树机制
**全新架构**：系统采用三层决策树替代传统的全局阈值方法，提供更精细的热信号分类。

#### 第一层：真实火点可见性检测 (local_fire_visible)
- **判定条件**：视野内真实火点数 > 0
- **数据来源**：基于fire_binary_map的精确匹配
- **置信度**：最高（直接观测到的真实火点）
- **应用场景**：确保在可见区域内的高置信度检测

#### 第二层：热传感器信号检测 (thermal_sensor_signal)  
- **判定条件**：当前位置 thermal_potential >= 0.50
- **数据来源**：经过稳健归一化的热势场
- **置信度**：中等（基于热势强度的间接证据）
- **应用场景**：在不可见区域提供弱引导，减少漏检

#### 第三层：综合热信号判定 (has_heat_signal)
- **判定逻辑**：local_fire_visible OR thermal_sensor_signal
- **输出结果**：布尔值表示是否存在热信号
- **优势**：结合两种证据源，提高检测鲁棒性

```mermaid
flowchart TD
Start(["进入热信号检测"]) --> GetPos["获取无人机位置(y,x)"]
GetPos --> LocalInfo["计算局部火情统计<br/>fire_count/boundary_count/nearest_distance"]
LocalInfo --> CheckVisible{"local_fire_visible?<br/>视野内真实火点>0?"}
CheckVisible --> |是| HasSignal["has_heat_signal=True"]
CheckVisible --> |否| ReadHeat["读取当前位置热势值"]
ReadHeat --> HeatCheck{"thermal_sensor_signal?<br/>热势>=0.50?"}
...
**图表来源**
- [rl_environment_baseline.py:671-690](file://environment_variables/environment_variables/rl_environment_baseline.py#L671-690)
- [信息转换.py:1070-1123](file://environment_variables/environment_variables/信息转换.py#L1070-1123)
...
**章节来源**
- [信息转换.py:559-602](file://environment_variables/environment_variables/信息转换.py#L559-602)
- [信息转换.py:759-819](file://environment_variables/environment_variables/信息转换.py#L759-L819)
- [rl_environment_baseline.py:671-690](file://environment_variables/environment_variables/rl_environment_baseline.py#L671-L690)
...
- **置信度评估**
  - **通过分层决策树形成多级置信度**："视野内真实火点计数"与"热势阈值"双重证据形成置信度；同时利用"最近火点距离""边界计数""平均/最大强度"辅助判别。
- **误报过滤机制**
  - 仅在视野内统计（圆形掩码），避免边缘效应；
...
## 结论
本系统通过**全新的分层决策树架构**，实现了local_fire_visible、thermal_sensor_signal和has_heat_signal三种状态层级的精确分类，结合稳健热势场与梯度引导，显著提升了火灾前沿识别的准确性与鲁棒性。**相比传统的全局阈值方法，新系统提供了更细粒度的置信度评估和更强的误报过滤能力**。课程学习与多源观测融合进一步增强了在噪声与复杂地形下的稳定性。建议在部署前执行热场健康诊断，并根据场景调整fire_threshold与near_prob退火策略。
