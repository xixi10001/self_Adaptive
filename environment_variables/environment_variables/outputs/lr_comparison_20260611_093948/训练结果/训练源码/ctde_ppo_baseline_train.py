...
class CurriculumManager:
    ...
    def update(self, success: bool, coverage: float) -> int:
    ...
    def _try_advance_stage3_target(self):
        ...
        if self._s3_target_eps >= min_eps and avg_coverage >= current_target * 0.85:
...
def train(config: Dict = None):
    ...
    with open(os.path.join(log_dir, "dataset_preflight.json"), "w", encoding="utf-8") as f:
        json.dump(_to_jsonable(preflight_counts), f, indent=2, ensure_ascii=False)
    ...
    for episode in range(1, config["total_episodes"] + 1):
        ...
        new_stage = curriculum.update(success, info["boundary_coverage"])
    ...
    if config["eval_after_train"]:
        ...
        for split in config["final_eval_splits"]:
            ...
            print(f"\n最终模型评估 | 数据划分={split}")
            split_results = evaluate(agent, split_config)
            split_path = os.path.join(log_dir, f"{split}_eval.json")
            _write_json(split_path, split_results)
            eval_summary["final"][split] = {
                "path": split_path,
                "stages": {
            ...
            if split == "generalization":
                generalization_results = split_results
                generalization_path = split_path
            ...
            print(f"{split}_eval={split_path}")

        best_val_path = best_model_paths.get("best_val")
        if config["evaluate_best_val_after_train"] and best_val_path and os.path.exists(best_val_path):
            ...
            best_val_agent.load(best_val_path)
            eval_summary["best_val"]["splits"] = {}
            ...
            for split in config["final_eval_splits"]:
                ...
                split_path = os.path.join(log_dir, f"best_val_{split}_eval.json")
                _write_json(split_path, split_results)
                eval_summary["best_val"]["splits"][split] = {
                    "path": split_path,
                    "stages": {
                        str(stage): _stage_summary(split_results, int(stage))
                        for stage in config["eval_stages"]
                ...
                print(f"best_val_{split}_eval={split_path}")
    ...
    if config["plot_after_train"]:
        figure_paths = _run_figure_scripts(
            output_dir,
            config,
            include_generalization=bool(config["eval_after_train"]),
        )
    ...
    if config["eval_after_train"]:
        ...
        print(f"评估结果={os.path.join(log_dir, 'eval_results.json')}")
