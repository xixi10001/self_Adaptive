...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.local_obs_dim = 17
        self.global_state_dim = 19
        ...
        if "intensity" in data:
            ...
            if getattr(self.env_data, "fire_binary_map", None) is not None:
                if self.env_data.fire_binary_map[y, x] == 0:
                    intensity = 0.0
            features["intensity_norm"] = intensity / max(float(norm["intensity_max"]), 1.0)

        if "dem" in data:
            dem = float(data["dem"][y, x])
            denom = max(float(norm["dem_max"] - norm["dem_min"]), 1.0)
            features["dem_norm"] = (dem - float(norm["dem_min"])) / denom

        if "slope" in data:
            features["slope_norm"] = float(data["slope"][y, x]) / max(float(norm["slope_max"]), 1.0)

        if "wind_speed" in data:
            features["wind_speed_norm"] = float(data["wind_speed"][y, x]) / max(
                float(norm["wind_speed_max"]), 1.0
            )
