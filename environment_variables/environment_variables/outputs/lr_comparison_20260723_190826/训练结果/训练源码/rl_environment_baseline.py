...
class CooperativeTaskCoordinator:
    ...
    def expire_messages(self, step: int) -> None:
        for drone_idx, message in enumerate(self.peer_messages):
            if message is None:
                continue
            age = int(step) - int(message["received_step"])
            if age < 0 or age >= self.peer_state_ttl:
        ...
        for drone_idx, seen in enumerate(self.boundary_last_seen):
            future = seen > int(step)
            if np.any(future):
                self.expired_boundary_report_count += int(np.count_nonzero(future))
                seen[future] = -1
    ...
    def _search_candidate_for_target(
        ...
        """Build a deployable SEARCH candidate from communicated map knowledge."""
        env = self.env
        pos = env.drone_positions[drone_idx].astype(np.float32)
        map_norm = max(float(np.linalg.norm(env.grid_size)), 1.0)
        target = np.array(
            [
                np.clip(float(target[0]), 0.0, env.grid_size[0] - 1.0),
                np.clip(float(target[1]), 0.0, env.grid_size[1] - 1.0),
            ],
            dtype=np.float32,
        )
        ty, tx = int(round(float(target[0]))), int(round(float(target[1])))
        y0, y1, x0, x1, mask = env._get_circular_window(ty, tx)
        known = env.agent_known_masks[drone_idx][y0:y1, x0:x1]
        novelty = float(
            np.count_nonzero(~known & mask) / max(np.count_nonzero(mask), 1)
        )
        delta = (target - pos) / map_norm
        valid = bool(
            novelty >= float(getattr(env, "search_target_min_novelty", 0.05))
            and np.linalg.norm(target - pos) >= max(1.0, env.vision_radius * 0.5)
        )
        return np.array(
    ...
    def _search_target_overlap(self, target0: np.ndarray, target1: np.ndarray) -> float:
        ...
        radius = max(float(self.env.vision_radius), 1.0)
        distance = float(np.linalg.norm(target0 - target1))
        if distance >= 2.0 * radius:
            return 0.0
        ratio = float(np.clip(distance / (2.0 * radius), 0.0, 1.0))
        intersection = (
            2.0 * radius * radius * np.arccos(ratio)
            - 0.5 * distance * np.sqrt(max(4.0 * radius * radius - distance * distance, 0.0))
        )
    ...
    def _candidate(self, drone_idx: int, role: int, step: int) -> np.ndarray:
        ...
        if role == self.TRACK:
            valid = (seen >= 0) & (ages >= 0) & (ages < self.track_report_ttl)
    ...
    def refresh_candidates(self, step: int) -> None:
        ...
        self._refresh_search_candidates(step)
        for drone_idx in range(self.env.num_drones):
            for role in range(1, self.NUM_ROLES):
...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    STAGE1_DISCOVERY_DEADLINE = 150
    STAGE1_TRACK_WINDOW = 20
    STAGE1_TRACK_REQUIRED = 14
    STAGE1_MIN_UNIQUE_BOUNDARY = 8
    STAGE2_TRACK_WINDOW = 15
    STAGE2_TRACK_REQUIRED = 10
    STAGE2_REACQUIRE_MIN_VISIBLE = 5
    STAGE2_SPAWN_QUANTILES = {
        "near": (0.20, 0.50),
        "medium": (0.40, 0.75),
        "far": (0.65, 1.00),
    }
    STAGE2_FINAL_SPAWN_MIX = (0.10, 0.25, 0.65)
    CONTROLLED_REFRESH_MIN_STEP = 300
    CONTROLLED_REFRESH_MAX_STEP = 420
    CONTROLLED_REFRESH_MIN_REMAINING = 150
    CONTROLLED_REFRESH_TIME_JUMP = 160
    CONTROLLED_REFRESH_MIN_DROP = 0.05
    CONTROLLED_REFRESH_TARGET = 0.50
    CONTROLLED_REFRESH_RECOVERY_STEPS = 120
    STAGE1_EXPLORE_REWARD_CAP = 2.0
    STAGE1_AREA_REWARD_CAP = 1.5
    STAGE1_SEARCH_REWARD_CAP = 1.5
    STAGE1_FRESH_REWARD_WEIGHT = 2.0
    STAGE1_FRESH_REWARD_STEP_CAP = 0.20
    ...
    def __init__(
        ...
        self.set_coverage_objective(coverage_objective)
        self.controlled_refresh_enabled = bool(controlled_refresh_enabled)
        self.mode = mode
        ...
        self.search_target_overlap_weight = max(0.0, float(search_target_overlap_weight))
        self.search_target_stickiness_weight = max(
            0.0, float(search_target_stickiness_weight)
        )
        self.search_target_min_novelty = float(
        ...
        self.pre_boundary_unique_novelty_weight = max(
            0.0, float(pre_boundary_unique_novelty_weight)
        )
        self.pre_boundary_overlap_weight = max(
            0.0, float(pre_boundary_overlap_weight)
        )
        self.pre_boundary_revisit_weight = max(
            0.0, float(pre_boundary_revisit_weight)
        )
        ...
        self.stage1_explore_reward_cap = self.STAGE1_EXPLORE_REWARD_CAP
        ...
        self.pre_boundary_revisit_steps = 0.0
        ...
        self.fresh_before_transition = 0.0
        self.fresh_after_action = 0.0
        self.fresh_after_refresh = 0.0
        self.coverage_action_gain_min = 0.0
        self.negative_action_gain_count = 0
    ...
    def _apply_uav_params(self):
        ...
        if self.curriculum_stage == 1:
            self.max_steps = min(self.max_steps, self.STAGE1_MAX_STEPS)
    ...
    def _pre_boundary_area_reward(self, new_area_cells: int) -> float:
        ...
        reward = float(np.clip(reward, 0.0, self.pre_boundary_area_gain_clip))
        if int(getattr(self, "curriculum_stage", 1)) == 1:
            total = float(getattr(self, "_episode_area_reward_total", 0.0))
            reward = min(reward, max(0.0, self.STAGE1_AREA_REWARD_CAP - total))
            self._episode_area_reward_total = total + reward
        return reward
    ...
    def _boundary_freshness_metrics(
        self, reference_step: Optional[int] = None
    ) -> Tuple[float, float]:
        if not self.boundary_points:
            return 0.0, 0.0
        reference_step = (
            int(self.step_count) if reference_step is None else int(reference_step)
        )
        tolerant_sum = 0.0
        fresh_sum = 0.0
    ...
    def _objective_coverage_rate(self) -> float:
        objective = getattr(self, "coverage_objective", "auto")
        if objective == "historical_unique":
            return self._historical_boundary_union_coverage_rate()
        if objective == "fresh":
    ...
    def _visible_region_mask(self, pos: np.ndarray) -> np.ndarray:
        y, x = int(pos[0]), int(pos[1])
        y_min, y_max, x_min, x_max, local_mask = self._get_circular_window(y, x)
        visible = np.zeros(self.grid_size, dtype=np.bool_)
        visible[y_min:y_max, x_min:x_max] = local_mask
        return visible

    def _pre_boundary_cooperative_search_reward(
        self,
        new_positions: List[np.ndarray],
    ) -> Tuple[List[float], Dict[str, float]]:
        """Reward team sensor-frontier progress without using hidden fire geometry."""
        ...
        known_before = np.logical_or.reduce(self.agent_observed_masks)
        visible_masks = [self._visible_region_mask(pos) for pos in new_positions]
        newly_visible = [visible & ~known_before for visible in visible_masks]
        team_new = np.logical_or.reduce(newly_visible)
        strip_area = max(float(2 * self.vision_radius + 1), 1.0)
        team_novelty = float(
            np.clip(np.count_nonzero(team_new) / (strip_area * num_agents), 0.0, 1.0)
        )

        unique_novelty = []
        revisit_fractions = []
        for drone_idx, new_mask in enumerate(newly_visible):
            peer_new = np.zeros(self.grid_size, dtype=np.bool_)
            for peer_idx, peer_mask in enumerate(newly_visible):
                if peer_idx != drone_idx:
                    peer_new |= peer_mask
            unique_novelty.append(
                float(
                    np.clip(
                        np.count_nonzero(new_mask & ~peer_new) / strip_area,
                        0.0,
                        1.0,
                    )
                )
            ...
            novelty_fraction = float(
                np.clip(np.count_nonzero(new_mask) / strip_area, 0.0, 1.0)
            )
            revisit_fractions.append(1.0 - novelty_fraction)

        overlap_ratio = 0.0
        if num_agents >= 2:
            intersection = np.logical_and.reduce(visible_masks)
            minimum_footprint = max(
                min(float(np.count_nonzero(mask)) for mask in visible_masks), 1.0
            )
            overlap_ratio = float(
                np.clip(np.count_nonzero(intersection) / minimum_footprint, 0.0, 1.0)
            )

        positive = [
            self.pre_boundary_team_novelty_weight * team_novelty / num_agents
            + self.pre_boundary_unique_novelty_weight * unique_novelty[idx]
            for idx in range(num_agents)
        ]
        positive_total = float(sum(positive))
        remaining = max(
            0.0,
            self.pre_boundary_reward_episode_cap
            - float(getattr(self, "_episode_pre_boundary_reward_total", 0.0)),
        )
        if positive_total > remaining and positive_total > 0.0:
            scale = remaining / positive_total
            positive = [value * scale for value in positive]
            positive_total = remaining
        self._episode_pre_boundary_reward_total = float(
            getattr(self, "_episode_pre_boundary_reward_total", 0.0)
            + positive_total
        )
        ...
        for idx in range(num_agents):
            rewards[idx] = (
                positive[idx]
                - overlap_penalty / num_agents
                - revisit_penalties[idx]
            )
        ...
        self.pre_boundary_revisit_steps = float(
            getattr(self, "pre_boundary_revisit_steps", 0.0)
            + sum(revisit_fractions)
        )
        self.team_overlap_sum = float(
            getattr(self, "team_overlap_sum", 0.0) + overlap_ratio * num_agents
        )
        return rewards, breakdown
    ...
    def reset(self) -> Dict:
        ...
        self._pending_refresh_step = -1
        self._pending_recovery_target = 0.0
        self.controlled_refresh_step = -1
        self.controlled_refresh_triggered = False
        self.controlled_refresh_opportunity = False
        self.controlled_refresh_recovered = False
        self.controlled_refresh_recovery_time = None
        self.controlled_refresh_pre_coverage = 0.0
        self.controlled_refresh_post_coverage = 0.0
        self._controlled_refresh_pending_step = -1
        self._major_refresh_time_offset = 0
        if (
            self.controlled_refresh_enabled
            and self.max_steps
            >= self.CONTROLLED_REFRESH_MIN_STEP
            + self.CONTROLLED_REFRESH_MIN_REMAINING
        ):
            latest = min(
                self.CONTROLLED_REFRESH_MAX_STEP,
    ...
    def _spawn_randomly(self, drone_idx: int) -> np.ndarray:
        if self.curriculum_stage == 2:
            ...
            q_min, q_max = self.STAGE2_SPAWN_QUANTILES[stratum]
            pos = self._spawn_by_boundary_quantile(q_min, q_max)
            self.spawn_modes.append(f"stage2_{stratum}")
            return pos
    ...
    def _spawn_by_boundary_quantile(self, q_min: float, q_max: float) -> np.ndarray:
        """Sample a legal training spawn from a per-scene distance quantile.

        Boundary geometry is used only by the environment reset/curriculum
        generator. It is never exposed through an actor observation.
        """
        height, width = self.grid_size
        margin = max(2, self.vision_radius // 2)
        boundary_mask = np.zeros(self.grid_size, dtype=np.bool_)
        for y, x in self.boundary_points:
            boundary_mask[int(y), int(x)] = True
        if not np.any(boundary_mask):
            raise RuntimeError("Cannot sample a stage-2 spawn without fire boundary points")

        boundary_distance = distance_transform_edt(~boundary_mask)
        fire_mask = np.asarray(self.env_data.fire_binary_map, dtype=np.bool_)
        fire_distance = distance_transform_edt(~fire_mask)
        legal = np.zeros(self.grid_size, dtype=np.bool_)
        legal[margin : height - margin, margin : width - margin] = True
        legal &= boundary_distance > float(self.vision_radius)
        legal &= fire_distance > float(self.vision_radius)

        legal_distances = boundary_distance[legal]
        if legal_distances.size == 0:
            raise RuntimeError("No legal stage-2 spawn cells remain after visibility masking")
        low = float(np.quantile(legal_distances, float(q_min)))
        high = float(np.quantile(legal_distances, float(q_max)))
        band = legal & (boundary_distance >= low) & (boundary_distance <= high)
        ...
        for index in order:
            ...
            if not self._too_close_to_existing_drones(pos):
                return pos
        raise RuntimeError(
            f"No separated stage-2 spawn for quantile=[{q_min:.2f}, {q_max:.2f}]"
        )
    ...
    def _pre_boundary_heat_progress_reward(
        self, old_pos: np.ndarray, new_pos: np.ndarray
    ) -> float:
        heat_signal = self._get_heat_signal_features(new_pos)
        if self.mask_thermal_below_signal and not heat_signal["has_heat_signal"]:
            return 0.0
        ...
        reward = min(coefficient * delta, cap)
        dense_remaining = max(
            0.0,
            float(getattr(self, "pre_boundary_reward_episode_cap", 4.0))
            - float(getattr(self, "_episode_pre_boundary_reward_total", 0.0)),
        )
        reward = min(reward, dense_remaining)
        if self.curriculum_stage == 1:
            remaining = max(
                0.0,
                self.STAGE1_SEARCH_REWARD_CAP - self._episode_search_reward_total,
            )
            reward = min(reward, remaining)
            self._episode_search_reward_total += reward
        self._episode_pre_boundary_reward_total = float(
            getattr(self, "_episode_pre_boundary_reward_total", 0.0) + reward
        )
        return float(reward)
    ...
    def _compute_reward(
        ...
        if cell not in self.visited_cells:
            ...
            if self.reward_profile == "persistent_boundary" and pre_boundary:
                explore_reward = 0.0
            else:
        ...
        if self.curriculum_stage != 1 and len(peers) > 1:
        ...
        if len(self.discovered_boundary) == 0:
            r_search = self._pre_boundary_heat_progress_reward(old_pos, new_pos)
            if r_search > 0.0:
                reward += r_search
    ...
    def _update_discovered_boundary(
        ...
        if drone_idx is not None:
            self._mark_boundary_seen(
                int(drone_idx),
                visible_boundary,
                self.step_count + 1 if observation_step is None else observation_step,
            )
    ...
    def _update_contact_curriculum_state(self) -> Tuple[bool, bool]:
        ...
        if getattr(self, "step_count", self.first_boundary_step + 1) <= self.first_boundary_step:
        ...
        if self.curriculum_stage == 1:
            ...
            self.stage1_tracking_success = bool(
                self.stage1_discovered_within_deadline
                and self.first_boundary_step > 0
                and len(self.stage1_tracking_window) >= self.STAGE1_TRACK_WINDOW
    ...
    def step(self, actions: List[int]) -> Tuple[Dict, List[float], bool, Dict]:
        transition_step = self.step_count + 1
        rewards = []
        ...
        if persistent_boundary:
            _, fresh_before_action = self._boundary_freshness_metrics(
                reference_step=transition_step
            )
        self.fresh_before_transition = float(fresh_before_action)
        ...
        cooperative_search_rewards = [0.0 for _ in range(n_act)]
        cooperative_search_breakdown = self._empty_reward_breakdown()
        if persistent_boundary and not discovered_before_action:
            (
                cooperative_search_rewards,
                cooperative_search_breakdown,
            ) = self._pre_boundary_cooperative_search_reward(new_positions)
        ...
        for i, action in enumerate(actions):
            ...
            reward += cooperative_search_rewards[i]
            ...
            if coordinator is not None:
                new_points, new_area_cells = self._update_discovered_boundary(
                    new_pos,
                    drone_idx=i,
                    observation_step=transition_step,
                )
            else:
            ...
            if pre_boundary and not persistent_boundary:
            ...
            if (
                ...
                and self.reward_profile not in {"novelty_search", "persistent_boundary"}
            ):
            ...
            if first_contact:
                self.first_boundary_step = transition_step
                if self.curriculum_stage == 1:
                    self.stage1_discovered_within_deadline = bool(
                        transition_step <= self.STAGE1_DISCOVERY_DEADLINE
                    )
                contact_bonus = (
                    3.0
                    if self.curriculum_stage == 1
        ...
        for key, value in cooperative_search_breakdown.items():
            step_reward_breakdown[key] += value
        ...
        if coordinator is not None:
            coordinator.sync_connected_agents(transition_step)
        ...
        if persistent_boundary:
            ...
            self.fresh_after_action = float(fresh_after_action)
            self.coverage_action_gain = float(
                fresh_after_action - fresh_before_action
            )
            self.coverage_action_gain_min = min(
                float(self.coverage_action_gain_min), self.coverage_action_gain
            )
            if self.coverage_action_gain < -1e-8:
                self.negative_action_gain_count += 1
                raise RuntimeError(
                    "boundary action gain became negative before boundary refresh"
                )
            self.coverage_action_gain = max(0.0, self.coverage_action_gain)
            if self.curriculum_stage == 1:
                team_fresh_reward = min(
                    self.STAGE1_FRESH_REWARD_WEIGHT * self.coverage_action_gain,
                    self.STAGE1_FRESH_REWARD_STEP_CAP,
                )
            else:
                fresh_weight = {
                    2: 3.0,
                }.get(int(self.curriculum_stage), self.fresh_coverage_gain_weight)
                team_fresh_reward = fresh_weight * self.coverage_action_gain
            rewards = [
        ...
        self.step_count = transition_step
        ...
        controlled_refresh_now = bool(
            getattr(self, "controlled_refresh_enabled", False)
            and not getattr(self, "controlled_refresh_triggered", False)
            and self.step_count == getattr(self, "controlled_refresh_step", -1)
        )
        objective_before_refresh = self._objective_coverage_rate()

        if self.step_count % 20 == 0 or controlled_refresh_now:
            self.boundary_refreshed = True
            if controlled_refresh_now:
                self.controlled_refresh_triggered = True
                self._major_refresh_time_offset += self.CONTROLLED_REFRESH_TIME_JUMP
            new_boundary = self.env_data.detect_fire_boundary(
                time_step=self.step_count
                + getattr(self, "_major_refresh_time_offset", 0),
        ...
        if persistent_boundary:
            ...
            self.fresh_after_refresh = float(fresh_coverage)
            self.coverage_refresh_drop = float(
                fresh_coverage - fresh_after_action
            )
        ...
        if controlled_refresh_now:
            objective_after_refresh = (
                float(fresh_coverage)
                if self.coverage_objective == "fresh"
                else float(self._objective_coverage_rate())
            )
            self.controlled_refresh_pre_coverage = float(objective_before_refresh)
            self.controlled_refresh_post_coverage = objective_after_refresh
            controlled_drop = objective_after_refresh - float(objective_before_refresh)
            self.controlled_refresh_opportunity = bool(
                objective_before_refresh >= self.CONTROLLED_REFRESH_TARGET
                and controlled_drop <= -self.CONTROLLED_REFRESH_MIN_DROP
            )
            if self.controlled_refresh_opportunity:
                self._controlled_refresh_pending_step = self.step_count
        if coordinator is not None:
        ...
        if getattr(self, "_pending_refresh_step", -1) >= 0:
            ...
            if objective_coverage >= self._pending_recovery_target:
                ...
                self._pending_refresh_step = -1
        if getattr(self, "_controlled_refresh_pending_step", -1) >= 0:
            elapsed = self.step_count - self._controlled_refresh_pending_step
            if objective_coverage >= self.CONTROLLED_REFRESH_TARGET:
                self.controlled_refresh_recovered = True
                self.controlled_refresh_recovery_time = int(elapsed)
                self._controlled_refresh_pending_step = -1
            elif elapsed >= self.CONTROLLED_REFRESH_RECOVERY_STEPS:
                self._controlled_refresh_pending_step = -1
        ...
        info = {
            ...
            "fresh_before_transition": float(
                getattr(self, "fresh_before_transition", 0.0)
            ),
            "fresh_after_action": float(getattr(self, "fresh_after_action", 0.0)),
            "fresh_after_refresh": float(
                getattr(self, "fresh_after_refresh", 0.0)
            ),
            "coverage_action_gain_min": float(
                getattr(self, "coverage_action_gain_min", 0.0)
            ),
            "negative_action_gain_count": int(
                getattr(self, "negative_action_gain_count", 0)
            ),
            ...
            "coverage_objective": getattr(self, "coverage_objective", "auto"),
            ...
            "stage1_discovered_within_deadline": bool(
                getattr(self, "stage1_discovered_within_deadline", False)
            ),
            ...
            "stage2_far_spawn_ratio": float(
                getattr(self, "stage2_far_spawn_ratio", 1.0)
            ),
            "stage2_spawn_mix": list(
                getattr(self, "stage2_spawn_mix", self.STAGE2_FINAL_SPAWN_MIX)
            ),
            ...
            "controlled_refresh_enabled": bool(
                getattr(self, "controlled_refresh_enabled", False)
            ),
            "controlled_refresh_step": int(
                getattr(self, "controlled_refresh_step", -1)
            ),
            "controlled_refresh_triggered": bool(
                getattr(self, "controlled_refresh_triggered", False)
            ),
            "controlled_refresh_opportunity": bool(
                getattr(self, "controlled_refresh_opportunity", False)
            ),
            "controlled_refresh_recovered": bool(
                getattr(self, "controlled_refresh_recovered", False)
            ),
            "controlled_refresh_recovery_time": getattr(
                self, "controlled_refresh_recovery_time", None
            ),
            "controlled_refresh_pre_coverage": float(
                getattr(self, "controlled_refresh_pre_coverage", 0.0)
            ),
            "controlled_refresh_post_coverage": float(
                getattr(self, "controlled_refresh_post_coverage", 0.0)
            ),
    ...
    def set_stage2_far_spawn_ratio(self, ratio: float):
        self.stage2_far_spawn_ratio = float(np.clip(float(ratio), 0.0, 1.0))

    def set_stage2_spawn_mix(self, mix: Tuple[float, float, float]):
        values = np.asarray(tuple(mix), dtype=np.float64)
        if values.shape != (3,) or np.any(values < 0.0) or float(np.sum(values)) <= 0.0:
            raise ValueError("stage2_spawn_mix must contain three non-negative weights")
        values = values / float(np.sum(values))
        self.stage2_spawn_mix = tuple(float(value) for value in values)

    def set_coverage_objective(self, objective: str):
        objective = str(objective)
        if objective not in {"auto", "historical_unique", "fresh", "exact"}:
            raise ValueError(f"Unsupported coverage objective: {objective}")
        self.coverage_objective = objective
    ...
    def get_env_info(self) -> Dict:
        return {
            ...
            "search_target_overlap_weight": self.search_target_overlap_weight,
            ...
            "termination_mode": self.termination_mode,
            "controlled_refresh_enabled": self.controlled_refresh_enabled,
