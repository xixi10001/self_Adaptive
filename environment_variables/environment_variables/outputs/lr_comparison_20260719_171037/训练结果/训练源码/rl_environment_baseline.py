...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.post_target_goal = float(np.clip(post_target_goal, 0.60, 0.80))
    ...
    def step(self, actions: List[int]) -> Tuple[Dict, List[float], bool, Dict]:
        ...
        if done:
            ...
            if self.termination_mode == "post_target_train" and done_reason == "horizon_reached":
    ...
    def set_post_target_goal(self, goal: float):
        self.post_target_goal = float(np.clip(float(goal), 0.60, 0.80))
