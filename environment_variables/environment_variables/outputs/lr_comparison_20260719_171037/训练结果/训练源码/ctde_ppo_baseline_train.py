...
DEFAULT_TRAIN_CONFIG = {
    ...
    "hierarchical_roles_enabled": False,
    "peer_state_ttl": 8,
    "track_report_ttl": 20,
    ...
    "mask_thermal_below_signal": False,
    "role_actor_lr": 2e-4,
    "role_critic_lr": 5e-4,
    "role_batch_size": 128,
...
class Stage4StepMixScheduler:
    ...
    def set_target_ratio(self, target_ratio: float, update_step: int):
        ...
        self._target_ratio = float(np.clip(target_ratio, 0.0, 1.0))
...
class CurriculumManager:
    ...
    STAGE3_NEAR_LADDER = [0.25, 0.10, 0.0]
    STAGE3_NEAR_VALIDATION_GATES = [
        (0.90, 0.10, 1.00),
        (0.95, 0.05, 0.25),
    ]
    STAGE3_NEAR_VALIDATION_PATIENCE = 2
    TERMINAL_FOCUS_EPISODES = 300  # 最后 300 回合强制评估条件
    STAGE4_CONTINUATION_RATIOS = [0.30, 0.50, 0.70, 1.00]
    STAGE4_EXTRA_STEPS = [100, 250, None, None]
    STAGE4_GOALS = [0.60, 0.65, 0.70, 0.70]
    STAGE4_ENTRY_MIN_EPS = 300
    STAGE4_MIN_REMAINING_EPISODES = 600
    ...
    def __init__(self, final_init_area_percent: float = 5.0, stage3_final_target: float = 0.60):
        ...
        self._s3_near_idx = 0
        self._near_validation_passes = 0
        ...
        self._stage4_validation_passes = 0
        self._stage4_guard_failures = 0
    ...
    def stage4_level(self) -> str:
        return ("4A", "4B", "4C", "4D")[self._stage4_level_idx]
    ...
    def can_enter_stage4(self, validation_summary: Dict) -> bool:
        return bool(
            ...
            and self.stage_episodes[3] >= self.STAGE4_ENTRY_MIN_EPS
            and self._s3_target_idx == len(self.STAGE3_TARGET_LADDER) - 1
            and self._s3_near_idx == len(self.STAGE3_NEAR_LADDER) - 1
            and float(validation_summary.get("success_rate", 0.0)) >= 0.60
    ...
    def update_stage4_validation(self, summary: Dict) -> bool:
        ...
        guard_passed = (
            ...
            >= float(baseline.get("boundary_found_rate", 0.0)) - 0.02
            and float(summary.get("target_reach_rate", 0.0))
            >= float(baseline.get("target_reach_rate", 0.0)) - 0.03
        )
        if not guard_passed:
            self._stage4_guard_failures += 1
            self._stage4_validation_passes = 0
            if self._stage4_guard_failures >= 2:
        ...
        self._stage4_validation_passes = (
            self._stage4_validation_passes + 1 if improvement_passed else 0
        )
        if self._stage4_validation_passes < 2:
            return False
    ...
    def update_stage3_validation(self, summary: Dict) -> bool:
        """Advance far-start difficulty only after consecutive validation passes."""
        if self.current_stage != 3:
            self._near_validation_passes = 0
            return False
        ...
        req_found, max_zero_timeout, max_revisit = self.STAGE3_NEAR_VALIDATION_GATES[
            self._s3_near_idx
        ]
        boundary_found = float(summary.get("boundary_found_rate", 0.0))
        zero_timeout = float(summary.get("zero_coverage_timeout_rate", 1.0))
        revisit = float(summary.get("mean_pre_boundary_revisit_ratio", 1.0))
        passed = (
            boundary_found >= req_found
            and zero_timeout <= max_zero_timeout
            and revisit <= max_revisit
        )
        self._near_validation_passes = (
            self._near_validation_passes + 1 if passed else 0
        )
        if self._near_validation_passes < self.STAGE3_NEAR_VALIDATION_PATIENCE:
            return False
        ...
        print(
            ...
            f"validation_boundary_found={boundary_found * 100:.1f}% | "
            f"zero_timeout={zero_timeout * 100:.1f}% | revisit={revisit * 100:.1f}%"
        )
        return True
...
def train(config: Dict = None):
    ...
    for episode in range(1, config["total_episodes"] + 1):
        ...
        if curriculum.current_stage == 4:
            stage4_mode = stage4_scheduler.choose_mode(agent.training_step)
            if stage4_mode == "continuation":
                env.termination_mode = "post_target_train"
                env.set_post_target_extra_steps(curriculum.stage4_extra_steps)
                env.set_post_target_goal(curriculum.stage4_goal)
                buffer_mode = 1
            else:
        ...
        while not done:
            train_role_policy = curriculum.current_stage == 4
        ...
        role_update_info = agent.update_roles() if curriculum.current_stage == 4 else {}

        if curriculum.current_stage == 4:
            stage4_scheduler.record(
                stage4_mode,
                episode_length,
                first_target_step=int(info.get("first_target_step", -1)),
            )
        ...
        if curriculum.current_stage == 4:
            stage4_stats = stage4_scheduler.stats(agent.training_step)
            training_log["stage4_level"].append(curriculum.stage4_level)
            training_log["stage4_mode"].append(stage4_mode)
            training_log["stage4_extra_steps"].append(curriculum.stage4_extra_steps)
        ...
        if episode % config["validation_interval"] == 0:
            ...
            if stage4_validation:
                ...
                stage4_score = float(
                    0.35 * val_summary["mean_tail100_coverage"]
                    + 0.30 * val_summary["mean_current_coverage_auc"]
                    + 0.20 * val_summary["mean_final_current_coverage"]
                    + 0.15 * hold60
                )
                ...
                if level_advanced:
                    if len(agent.buffer) >= agent.min_update_batch_size:
                        ...
                        agent.buffer.clear()
                    stage4_scheduler.set_target_ratio(
                ...
                if curriculum._stage4_stop_requested:
                    stop_stage4_training = True
                    print("  [stage4 guard] 连续两次低于搜索保护线，提前停止阶段4")
                ...
                is_best_val = (
                    ...
                    and curriculum.current_stage == 3
                    and curriculum.stage_episodes[3] >= 300
                    and curriculum._terminal_focus_active
                ...
                if curriculum.update_stage3_validation(val_summary):
                    if len(agent.buffer) >= agent.min_update_batch_size:
                        pending_update_info = agent.update(force=True)
                        if pending_update_info.get("actor_loss", 0.0) != 0.0:
                            update_info = pending_update_info
