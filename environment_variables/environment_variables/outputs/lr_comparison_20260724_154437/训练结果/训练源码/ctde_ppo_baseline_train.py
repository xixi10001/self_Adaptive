...
DEFAULT_TRAIN_CONFIG = {
    ...
    "assigned_boundary_gain_weight": 6.0,
    "role_switch_penalty": 0.02,
    ...
    "post_target_step_penalty": -0.005,
...
def normalize_training_config(config: Dict = None) -> Dict:
    ...
    for key in [
        "search_target_overlap_weight",
        "search_target_stickiness_weight",
        "pre_boundary_team_novelty_weight",
        "pre_boundary_unique_novelty_weight",
...
def _append_episode_diagnostics(training_log: Dict, info: Dict) -> None:
    ...
    for key in [
        ...
        "fresh_before_transition",
        "fresh_after_action",
        "fresh_after_refresh",
        "coverage_action_gain_min",
        "negative_action_gain_count",
        ...
        "stage2_contact_success",
        "stage2_reacquire_success",
        "stage2_tracking_steps",
        "major_refresh_count",
        "refresh_recovery_successes",
        "mean_refresh_recovery_time",
        "controlled_refresh_step",
        "controlled_refresh_triggered",
        "controlled_refresh_opportunity",
        "controlled_refresh_recovered",
...
class Stage4HorizonMixScheduler:
    """Balance stage-4 horizon modes by realized environment steps."""

    PHASE_MIXES = {
        "early": {"extra150": 0.50, "extra300": 0.30, "full": 0.20},
        "middle": {"extra150": 0.25, "extra300": 0.35, "full": 0.40},
        "late": {"extra300": 0.20, "full": 0.80},
    }
    EXTRA_STEPS = {"extra150": 150, "extra300": 300, "full": None}

    def __init__(self):
        self.phase = "early"
        self.mode_steps = {mode: 0 for mode in self.EXTRA_STEPS}
        self.post_target_steps = 0

    def set_phase(self, phase: str):
        if phase not in self.PHASE_MIXES:
            raise ValueError(f"Unsupported stage-4 horizon phase: {phase}")
        self.phase = str(phase)

    def choose_mode(self) -> str:
        mix = self.PHASE_MIXES[self.phase]
        active_steps = sum(self.mode_steps[mode] for mode in mix)
        if active_steps <= 0:
            return max(mix, key=mix.get)
        debts = {
            mode: float(weight * active_steps - self.mode_steps[mode])
            for mode, weight in mix.items()
        }
        return max(debts, key=debts.get)

    def record(self, mode: str, episode_steps: int, first_target_step: int = -1):
        if mode not in self.mode_steps:
            raise ValueError(f"Unsupported stage-4 horizon mode: {mode}")
        episode_steps = max(0, int(episode_steps))
        self.mode_steps[mode] += episode_steps
        if int(first_target_step) > 0:
            self.post_target_steps += max(0, episode_steps - int(first_target_step))
    ...
    def stats(self) -> Dict[str, object]:
        ...
        return {
            "phase": self.phase,
            "target_step_mix": dict(mix),
            "realized_step_mix": {
                mode: self.mode_steps[mode] / max(total, 1)
                for mode in self.mode_steps
            },
            "post_target_ratio": self.post_target_steps / max(total, 1),
...
class CurriculumManager:
    ...
    STAGE2_MAX_EPISODES = 1000
    STAGE2_SPAWN_MIXES = {
        "early": (0.60, 0.30, 0.10),
        "middle": (0.25, 0.50, 0.25),
        "late": (0.10, 0.25, 0.65),
        "recovery": (0.45, 0.40, 0.15),
    }

    STAGE3_SUBSTAGES = ("3A", "3B", "3C", "3D")
    STAGE3_TARGET_MIN_EPS = (150, 200, 250, 300)
    STAGE3_TARGET_SOFT_MAX_EPS = (200, 250, 300, 350)
    STAGE3_TARGET_REACH_RATES = (0.70, 0.65, 0.60, 0.55)
    STAGE3_CONTINUATION_RATIOS = (0.10, 0.15, 0.20, 0.30)
    STAGE3_EXTRA_STEPS = (50, 75, 100, 150)

    STAGE4_PHASE_ENDS = (200, 450)
    STAGE4_FINAL_GATE_MIN_EPISODES = 600

    def __init__(
        self,
        final_init_area_percent: float = 5.0,
        stage3_final_target: float = 0.60,
    ):
        ...
        self._stage3_budget_caps = list(self.STAGE3_TARGET_SOFT_MAX_EPS)
        self._stage3_guard_baseline = None
        self._stage3_ready_for_stage4 = False
        self._stage4_budget_cap = self.STAGE4_MIN_REMAINING_EPISODES
        self._stage4_baseline = None
        self._stage4_guard_history = deque(maxlen=self.VALIDATION_POOL_SIZE)
        self._stage4_final_history = deque(maxlen=self.VALIDATION_POOL_SIZE)
        self._stage4_stop_requested = False
        self._stage4_recovery_requested = False
        self._validation_summaries = deque(maxlen=self.VALIDATION_POOL_SIZE)
        self._last_validation_gate = {}
        self._last_pooled_summary = {}
        ...
        self.curriculum_failure_reason = None
    ...
    @property
    def stage2_phase(self) -> str:
        episodes = self.stage_episodes[2]
        if episodes < self._stage2_recovery_until:
            return "recovery"
        if episodes < 250:
            return "early"
        if episodes < 500:
    ...
    @property
    def stage2_spawn_mix(self) -> Tuple[float, float, float]:
        return self.STAGE2_SPAWN_MIXES[self.stage2_phase]

    @property
    def stage2_far_spawn_ratio(self) -> float:
        return float(self.stage2_spawn_mix[2])
    ...
    @property
    def stage4_phase(self) -> str:
        episodes = self.stage_episodes[4]
        if episodes < self.STAGE4_PHASE_ENDS[0]:
            return "early"
        if episodes < self.STAGE4_PHASE_ENDS[1]:
            return "middle"
        return "late"

    @property
    def stage4_level(self) -> str:
        return self.stage4_phase
    ...
    def update(
        ...
        success: bool,
        coverage: float,
        zero_coverage_timeout: bool = False,
    ) -> int:
        ...
        self.stage_zero_timeout_rates[stage].append(
            1.0 if zero_coverage_timeout else 0.0
        )
        return self.current_stage

    def _minimum_episodes_reached(self) -> bool:
        if self.current_stage == 1:
            required = self.STAGE1_MIN_EPISODES
        elif self.current_stage == 2:
            required = self.STAGE2_MIN_EPISODES
        elif self.current_stage == 3:
            required = self.STAGE3_TARGET_MIN_EPS[self._s3_target_idx]
        else:
            required = self.STAGE4_FINAL_GATE_MIN_EPISODES
        return self.substage_episodes[self.current_substage] >= required

    @property
    def base_current_budget_cap(self) -> int:
        if self.current_stage == 1:
            return self.STAGE1_MAX_EPISODES
        if self.current_stage == 2:
            return self.STAGE2_MAX_EPISODES
        if self.current_stage == 3:
            return self._stage3_budget_caps[self._s3_target_idx]
        return self._stage4_budget_cap

    @property
    def current_budget_cap(self) -> int:
        return int(self.base_current_budget_cap)

    @staticmethod
    def _mean_rate(summaries: List[Dict], key: str) -> float:
        weighted = [
            (float(summary.get(key, 0.0)), int(summary.get("episodes", 0)))
            for summary in summaries
        ]
        denominator = sum(max(count, 0) for _, count in weighted)
        if denominator > 0:
            return float(
                sum(value * max(count, 0) for value, count in weighted) / denominator
            )
        return float(np.mean([value for value, _ in weighted])) if weighted else 0.0

    def _pooled_validation_summary(self) -> Dict:
        summaries = list(self._validation_summaries)
        if not summaries:
            return {}
        pooled = copy.deepcopy(summaries[-1])
        rate_keys = {
            "boundary_found_rate",
            "zero_discovery_timeout_rate",
            "zero_coverage_timeout_rate",
            "success_rate",
        ...
        stage1_found = sum(
            int(summary.get("stage1_deadline_found_count", 0))
            for summary in summaries
        )
        stage1_tracked = sum(
            int(summary.get("stage1_tracking_success_count", 0))
            for summary in summaries
        )
        if stage1_found > 0:
            pooled["stage1_tracking_given_found_rate"] = float(
                stage1_tracked / stage1_found
            )
        stage2_found = sum(
            int(summary.get("stage2_found_count", 0)) for summary in summaries
        )
        stage2_contact = sum(
            int(summary.get("stage2_contact_success_count", 0))
            for summary in summaries
        )
        if stage2_found > 0:
            pooled["stage2_contact_given_found_rate"] = float(
                stage2_contact / stage2_found
            )
        pooled["episodes"] = int(
            sum(int(summary.get("episodes", 0)) for summary in summaries)
        )
        for key in {
            "median_first_boundary_step",
            "median_first_boundary_step_far",
            "median_unique_boundary_cells",
            ...
            pooled[key] = float(np.median(values)) if values else None
        return pooled

    def validation_gate_status(self, summary: Dict) -> Dict[str, Dict[str, object]]:
        ...
        if self.current_stage == 1:
            unique_value = summary.get("median_unique_boundary_cells_given_found")
            if unique_value is None:
                unique_value = summary.get("median_unique_boundary_cells", 0.0)
            if unique_value is None:
                unique_value = 0.0
            checks = {
                "boundary_found_by_150": (
                    float(summary.get("stage1_discovery_within_deadline_rate", found)),
                    0.85,
                    ">=",
                ),
                "tracking_given_found": (
                    float(summary.get("stage1_tracking_given_found_rate", 0.0)),
                    0.70,
                    ">=",
                ),
                "zero_discovery_timeout": (zero_timeout, 0.15, "<="),
                "median_unique_boundary_cells": (
                    float(unique_value),
                    8.0,
                    ">=",
            ...
            far_median = summary.get(
                "median_first_boundary_step_far",
                summary.get("median_first_boundary_step"),
            )
            checks = {
                "boundary_found": (found, 0.75, ">="),
                "contact_given_found": (
                    float(summary.get("stage2_contact_given_found_rate", 0.0)),
                    0.70,
                    ">=",
                ),
                "zero_discovery_timeout": (zero_timeout, 0.25, "<="),
                "far_median_first_boundary_step": (
                    None if far_median is None else float(far_median),
                    240.0,
                    "<=",
                ),
            }
            ...
            checks = {
                "boundary_found": (found, 0.80, ">="),
                "target_success": (
                    float(summary.get("success_rate", 0.0)),
                    self.STAGE3_TARGET_REACH_RATES[self._s3_target_idx],
                    ">=",
                ),
                "zero_discovery_timeout": (zero_timeout, 0.20, "<="),
            }
            if self._stage3_guard_baseline is not None:
                checks["team_overlap_delta"] = (
                    float(summary.get("mean_team_overlap_ratio", 0.0))
                    - float(
                        self._stage3_guard_baseline.get(
                            "mean_team_overlap_ratio", 0.0
                        )
                    ),
                    0.05,
                    "<=",
                )
                checks["invalid_action_delta"] = (
                    float(summary.get("mean_invalid_action_count", 0.0))
                    - float(
                        self._stage3_guard_baseline.get(
    ...
    def _validation_passed(self, summary: Dict) -> bool:
        self._last_validation_gate = self.validation_gate_status(summary)
        return bool(
            self._last_validation_gate
            and all(check["passed"] for check in self._last_validation_gate.values())
        )

    def _allocate_stage3_budgets(self):
        available = (
            self.GLOBAL_EPISODE_BUDGET
            - self.stage_episodes[1]
            - self.stage_episodes[2]
            - self.STAGE4_MIN_REMAINING_EPISODES
        )
        minimum_total = sum(self.STAGE3_TARGET_MIN_EPS)
        soft_total = sum(self.STAGE3_TARGET_SOFT_MAX_EPS)
        available = int(np.clip(available, minimum_total, soft_total))
        caps = list(self.STAGE3_TARGET_MIN_EPS)
        remaining = available - minimum_total
        for index, soft_cap in enumerate(self.STAGE3_TARGET_SOFT_MAX_EPS):
            extra = min(remaining, soft_cap - caps[index])
            caps[index] += int(extra)
            remaining -= int(extra)
        self._stage3_budget_caps = caps

    def _advance_validated_substage(self, pooled: Dict) -> str:
        old_substage = self.current_substage
        self._validation_summaries.clear()
        if self.current_stage == 1:
            self.current_stage = 2
        elif self.current_stage == 2:
            self.current_stage = 3
            self._allocate_stage3_budgets()
        elif self._s3_target_idx < len(self.STAGE3_TARGET_LADDER) - 1:
            self._stage3_guard_baseline = copy.deepcopy(pooled)
    ...
    def _print_validation_gate(self) -> None:
        print(
            f"  [课程门槛] substage={self.current_substage} | "
            f"pooled_validations={len(self._validation_summaries)}/"
            f"{self.VALIDATION_POOL_SIZE}"
        )
        for name, check in self._last_validation_gate.items():
            actual = check["actual"]
            threshold = float(check["threshold"])
            operator = str(check["operator"])
            percentage = name not in {
                "median_unique_boundary_cells",
                "far_median_first_boundary_step",
                "team_overlap_delta",
            ...
            if actual is None:
                ...
                actual_text = f"{float(actual) * 100:.1f}%"
            else:
                actual_text = f"{float(actual):.1f}"
            threshold_text = (
                f"{threshold * 100:.1f}%" if percentage else f"{threshold:.1f}"
            )
            result = "PASS" if check["passed"] else "FAIL"
            print(
                f"    {name}: {actual_text} {operator} {threshold_text} | {result}"
            )
        print(
            f"    episodes={self.substage_episodes[self.current_substage]}/"
            f"{self.current_budget_cap} | stage2_phase={self.stage2_phase} | "
            f"stage2_mix={self.stage2_spawn_mix}"
        )

    def _stage2_score(self, pooled: Dict) -> float:
        return float(
            0.45 * pooled.get("boundary_found_rate", 0.0)
            + 0.35 * pooled.get("stage2_contact_given_found_rate", 0.0)
    ...
    def _handle_stage2_soft_budget(self, pooled: Dict):
        if self.stage_episodes[2] < self.STAGE2_SOFT_EPISODES:
            return
        score = self._stage2_score(pooled)
        if score >= self._stage2_best_score + 0.02:
            self._stage2_best_score = score
            self._stage2_plateau_count = 0
            return
        self._stage2_plateau_count += 1
        if self._stage2_plateau_count >= 3:
            self._stage2_recovery_until = min(
                self.STAGE2_MAX_EPISODES,
                self.stage_episodes[2] + 100,
            )
            self._stage2_plateau_count = 0
            print(
                "  [Stage2平台恢复] 连续三次验证无显著改善，"
    ...
    def update_validation(self, summary: Dict) -> bool:
        ...
        pooled = self._pooled_validation_summary()
        self._last_pooled_summary = copy.deepcopy(pooled)
        passed = self._validation_passed(pooled)
        self._print_validation_gate()
        enough_validation = (
            len(self._validation_summaries) >= self.VALIDATION_POOL_SIZE
        )
        if enough_validation and self._minimum_episodes_reached() and passed:
            old_substage = self._advance_validated_substage(pooled)
            next_label = (
                "Stage4准入就绪"
                if self._stage3_ready_for_stage4
                else self.current_substage
            )
            print(
                f"\n  [验证课程] {old_substage} -> {next_label} | "
                f"pooled_episodes={pooled.get('episodes', 0)}"
            )
            return True
    ...
    def can_enter_stage4(self, validation_summary: Dict) -> bool:
        summary = self._last_pooled_summary or validation_summary
        return bool(
            self.current_stage == 3
            and self._stage3_ready_for_stage4
            and self._s3_target_idx == len(self.STAGE3_TARGET_LADDER) - 1
            and float(summary.get("success_rate", 0.0)) >= 0.55
            and float(summary.get("boundary_found_rate", 0.0)) >= 0.80
            and float(summary.get("zero_discovery_timeout_rate", 1.0)) <= 0.20
        )

    def enter_stage4(self, full_horizon_baseline: Dict):
        used = sum(self.stage_episodes[stage] for stage in (1, 2, 3))
        self._stage4_budget_cap = max(
            self.STAGE4_MIN_REMAINING_EPISODES,
            self.GLOBAL_EPISODE_BUDGET - used,
        )
        self.current_stage = 4
        self._stage4_guard_history.clear()
        self._stage4_final_history.clear()
        self._stage4_stop_requested = False
        self._stage4_recovery_requested = False
    ...
    def update_stage4_validation(self, summary: Dict) -> bool:
        if self.current_stage != 4 or self._stage4_baseline is None:
            return False
        guard_passed = bool(
            float(summary.get("boundary_found_rate", 0.0)) >= 0.80
            and float(summary.get("zero_discovery_timeout_rate", 1.0)) <= 0.20
        )
        self._stage4_guard_history.append(guard_passed)
        if (
            len(self._stage4_guard_history) == self.VALIDATION_POOL_SIZE
        ...
        final_passed = bool(
            guard_passed
            and float(summary.get("target_reach_rate", 0.0)) >= 0.55
            and float(summary.get("mean_tail100_coverage", 0.0)) >= 0.55
            and int(summary.get("controlled_refresh_opportunities", 0)) >= 30
            and float(summary.get("controlled_refresh_recovery_rate", 0.0)) >= 0.60
        )
        self._stage4_final_history.append(final_passed)
        if (
            self.stage_episodes[4] >= self.STAGE4_FINAL_GATE_MIN_EPISODES
            and len(self._stage4_final_history) == self.VALIDATION_POOL_SIZE
            and sum(self._stage4_final_history) >= 2
        ):
            self._stage4_stop_requested = True
        return False
    ...
    def state_dict(self) -> Dict:
        return {
            ...
            "stage4_budget_cap": self._stage4_budget_cap,
            "stage3_ready_for_stage4": self._stage3_ready_for_stage4,
            "stage4_baseline": copy.deepcopy(self._stage4_baseline),
            "curriculum_failed": self.curriculum_failed,
            "curriculum_failure_reason": self.curriculum_failure_reason,
            "last_validation_gate": copy.deepcopy(self._last_validation_gate),
            "last_pooled_summary": copy.deepcopy(self._last_pooled_summary),
        }

    def get_stage_info(self) -> Dict:
        stage = self.current_stage
        success_rate = (
            float(np.mean(self.stage_success_rates[stage]))
            if self.stage_success_rates[stage]
            else 0.0
        )
        return {
            "stage": stage,
            ...
            "stage2_spawn_mix": self.stage2_spawn_mix,
            "stage2_phase": self.stage2_phase,
            "transferable_budget": 0,
            "stage4_level": self.stage4_phase if stage == 4 else None,
            "stage4_continuation_ratio": 1.0 if stage == 4 else None,
            "stage4_extra_steps": None,
            "stage4_goal": self.stage4_goal if stage == 4 else None,
            "substage_budget_cap": self.current_budget_cap,
            "curriculum_failed": self.curriculum_failed,
            "curriculum_failure_reason": self.curriculum_failure_reason,
        }
...
def _has_saved_generalization_data(result_dir: str) -> bool:
    for _, _, files in os.walk(os.path.abspath(result_dir)):
        for name in files:
            lower = name.lower()
            if lower.endswith(".csv") and (
                lower.startswith("detailed_eval_") or "eval" in lower
            ):
                return True
            if lower.endswith(".json") and (
                lower.startswith("_result_")
                or lower.startswith("eval_results")
                or lower.startswith("generalization_results")
                or ("eval" in lower and "results" in lower)
            ):
                return True
    return False
...
def _run_figure_scripts(
    ...
    if include_generalization and not _has_saved_generalization_data(result_dir):
        print("跳过泛化评估图表：课程未生成可用的泛化评估数据")
    elif include_generalization:
...
def _build_experiment_metadata(
    ...
    return {
        ...
        "cooperative_search": {
            "target_overlap_weight": float(config["search_target_overlap_weight"]),
            "target_stickiness_weight": float(
                config["search_target_stickiness_weight"]
            ),
            ...
            "unique_novelty_weight": float(
                config["pre_boundary_unique_novelty_weight"]
            ),
            "overlap_weight": float(config["pre_boundary_overlap_weight"]),
            "revisit_weight": float(config["pre_boundary_revisit_weight"]),
        ...
        "curriculum_budget": {
            ...
            "stage2_spawn_mixes": copy.deepcopy(
                CurriculumManager.STAGE2_SPAWN_MIXES
            ),
            "stage3_min": list(CurriculumManager.STAGE3_TARGET_MIN_EPS),
            "stage3_soft_max": list(
                CurriculumManager.STAGE3_TARGET_SOFT_MAX_EPS
            ),
            "stage4_reserved": CurriculumManager.STAGE4_MIN_REMAINING_EPISODES,
            "shared_adaptive_budget": 200,
        },
        "stage1_tracking_gate": {
            "max_steps": FireSearchBaselineEnvironment.STAGE1_MAX_STEPS,
            "discovery_deadline": (
                FireSearchBaselineEnvironment.STAGE1_DISCOVERY_DEADLINE
            ),
            "window": FireSearchBaselineEnvironment.STAGE1_TRACK_WINDOW,
            "required_visible_steps": FireSearchBaselineEnvironment.STAGE1_TRACK_REQUIRED,
            "minimum_unique_boundary_cells": FireSearchBaselineEnvironment.STAGE1_MIN_UNIQUE_BOUNDARY,
        },
        "stage2_contact_gate": {
            "window": FireSearchBaselineEnvironment.STAGE2_TRACK_WINDOW,
            "required_visible_steps": FireSearchBaselineEnvironment.STAGE2_TRACK_REQUIRED,
            "reacquire_minimum_visible_steps": FireSearchBaselineEnvironment.STAGE2_REACQUIRE_MIN_VISIBLE,
        },
...
def _summarize_full_horizon_records(records: List[Dict], target: float) -> Dict:
    ...
    return {
        ...
        "controlled_refresh_opportunities": int(
            sum(bool(record.get("controlled_refresh_opportunity", False)) for record in records)
        ),
        "controlled_refresh_recovery_rate": float(
            sum(bool(record.get("controlled_refresh_recovered", False)) for record in records)
            / max(
                sum(
                    bool(record.get("controlled_refresh_opportunity", False))
                    for record in records
                ),
                1,
            )
        ),
        "controlled_refresh_mean_recovery_time": (
            float(
                np.mean(
                    [
                        float(record["controlled_refresh_recovery_time"])
                        for record in records
                        if record.get("controlled_refresh_recovery_time") is not None
                    ]
                )
            )
            if any(
                record.get("controlled_refresh_recovery_time") is not None
                for record in records
            )
            else None
        ),
...
def _persistent_env_kwargs(
    ...
    return {
        ...
        "search_target_overlap_weight": config["search_target_overlap_weight"],
...
def _new_validation_log() -> Dict[str, List]:
    keys = (
        ...
        "val_zero_discovery_timeout_rate",
        ...
        "val_median_first_boundary_step_far",
        "val_median_unique_boundary_cells",
        "val_median_unique_boundary_cells_given_found",
        "val_mean_return_success",
        "val_mean_return_found_failed",
        "val_success_found_failed_return_margin",
        "val_negative_action_gain_count",
        "curriculum_gate",
        ...
        "stage4_tail100",
        "stage4_hold60",
        "stage4_controlled_refresh_opportunities",
        "stage4_controlled_refresh_recovery_rate",
        "stage4_controlled_refresh_mean_recovery_time",
    )
...
def train(config: Dict = None):
    ...
    stage4_horizon_scheduler = Stage4HorizonMixScheduler()
    ...
    env.set_stage2_spawn_mix(curriculum.stage2_spawn_mix)
    ...
    training_log = {
        ...
        "stage4_target_step_mix": [],
        "stage4_realized_step_mix": [],
    }
    ...
    for episode in range(1, config["total_episodes"] + 1):
        ...
        env.set_curriculum_substage(curriculum.current_substage)
        env.set_stage2_far_spawn_ratio(curriculum.stage2_far_spawn_ratio)
        env.set_stage2_spawn_mix(curriculum.stage2_spawn_mix)
        env.stage_targets[3] = curriculum.current_stage3_target
        env.stage3_near_prob = 0.0
        env.set_coverage_objective(
            "fresh"
        ...
        env.set_controlled_refresh_enabled(curriculum.current_stage == 4)
        stage4_mode = "baseline"
        buffer_mode = 0
        if curriculum.current_stage == 3:
            continuation_scheduler.set_target_ratio(
                curriculum.stage3_continuation_ratio, agent.training_step
            )
            ...
            if stage4_mode == "continuation":
                ...
                env.set_post_target_extra_steps(curriculum.stage3_extra_steps)
                env.set_post_target_goal(curriculum.current_stage3_target)
                buffer_mode = 1
            else:
                env.termination_mode = "target_stop"
                env.set_post_target_extra_steps(None)
        elif curriculum.current_stage == 4:
            stage4_horizon_scheduler.set_phase(curriculum.stage4_phase)
            stage4_mode = stage4_horizon_scheduler.choose_mode()
            env.termination_mode = "post_target_train"
            env.set_post_target_extra_steps(
                Stage4HorizonMixScheduler.EXTRA_STEPS[stage4_mode]
            )
            env.set_post_target_goal(0.60)
            buffer_mode = 1
        else:
            env.termination_mode = "target_stop"
            env.set_post_target_extra_steps(None)
        ...
        if curriculum.current_stage == 3:
            ...
            stage4_horizon_scheduler.record(
                stage4_mode,
                episode_length,
                first_target_step=int(info.get("first_target_step", -1)),
            )
        ...
        task_score = _task_score(
            ...
            success,
            episode_length,
            int(info.get("max_steps", config["max_steps"])),
        )
        ...
        training_log["stage2_far_spawn_ratio"].append(
            float(curriculum.stage2_far_spawn_ratio)
        )
        training_log["stage2_spawn_mix"].append(list(curriculum.stage2_spawn_mix))
        training_log["stage2_phase"].append(curriculum.stage2_phase)
        ...
        if curriculum.current_stage == 3:
            ...
            training_log["stage4_extra_steps"].append(curriculum.stage3_extra_steps)
            ...
            training_log["stage4_target_step_mix"].append({})
            training_log["stage4_realized_step_mix"].append({})
        elif curriculum.current_stage == 4:
            horizon_stats = stage4_horizon_scheduler.stats()
            training_log["stage4_level"].append(curriculum.stage4_phase)
            training_log["stage4_mode"].append(stage4_mode)
            training_log["stage4_extra_steps"].append(
                Stage4HorizonMixScheduler.EXTRA_STEPS[stage4_mode]
            )
            training_log["stage4_target_continuation_ratio"].append(1.0)
            training_log["stage4_realized_continuation_ratio"].append(1.0)
            training_log["stage4_realized_post_target_ratio"].append(
                horizon_stats["post_target_ratio"]
            )
            training_log["stage4_target_step_mix"].append(
                horizon_stats["target_step_mix"]
            )
            training_log["stage4_realized_step_mix"].append(
                horizon_stats["realized_step_mix"]
            )
        else:
            training_log["stage4_level"].append(None)
        ...
        if episode % config["validation_interval"] == 0:
            ...
            validated_stage2_spawn_mix = curriculum.stage2_spawn_mix
            validated_stage2_phase = curriculum.stage2_phase
            ...
            if stage4_validation:
                ...
                stage4_score = float(
                    ...
                    + 0.15
                    * val_summary.get("controlled_refresh_recovery_rate", 0.0)
                    + 0.10 * val_summary.get("target_reach_rate", 0.0)
                )
                guard_passed = (
                    float(val_summary.get("boundary_found_rate", 0.0)) >= 0.80
                    and float(
                        val_summary.get("zero_discovery_timeout_rate", 1.0)
                    )
                    <= 0.20
                )
                is_best_val = bool(
                ...
                curriculum.update_stage4_validation(val_summary)
                ...
                stage4_controlled_opportunities = int(
                    val_summary.get("controlled_refresh_opportunities", 0)
                )
                stage4_controlled_recovery_rate = float(
                    val_summary.get("controlled_refresh_recovery_rate", 0.0)
                )
                stage4_controlled_recovery_time = val_summary.get(
                    "controlled_refresh_mean_recovery_time"
                )
            else:
                ...
                stage4_tail100 = 0.0
                stage4_hold60 = 0.0
                stage4_controlled_opportunities = 0
                stage4_controlled_recovery_rate = 0.0
            ...
            validation_log["stage2_far_spawn_ratio"].append(
                float(validated_stage2_far_ratio)
            )
            validation_log["stage2_spawn_mix"].append(
                list(validated_stage2_spawn_mix)
            )
            validation_log["stage2_phase"].append(validated_stage2_phase)
            ...
            validation_log["val_zero_discovery_timeout_rate"].append(
                float(val_summary.get("zero_discovery_timeout_rate", 0.0))
            )
            validation_log["val_boundary_found_rate"].append(val_boundary_found_rate)
            validation_log["val_stable_tracking_success_rate"].append(
                float(val_summary.get("stable_tracking_success_rate", 0.0))
            )
            validation_log["val_stage1_discovery_within_deadline_rate"].append(
                float(
                    val_summary.get(
                        "stage1_discovery_within_deadline_rate", 0.0
                    )
                )
            )
            ...
            validation_log["val_stage2_reacquire_success_rate"].append(
                float(val_summary.get("stage2_reacquire_success_rate", 0.0))
            )
            validation_log["val_median_first_boundary_step"].append(
                val_summary.get("median_first_boundary_step")
            )
            validation_log["val_median_first_boundary_step_far"].append(
                val_summary.get("median_first_boundary_step_far")
            )
            validation_log["val_median_unique_boundary_cells"].append(
                val_summary.get("median_unique_boundary_cells")
            )
            validation_log[
                "val_median_unique_boundary_cells_given_found"
            ].append(
                val_summary.get("median_unique_boundary_cells_given_found")
            )
            validation_log["val_mean_return_success"].append(
                val_summary.get("mean_return_success")
            )
            validation_log["val_mean_return_found_failed"].append(
                val_summary.get("mean_return_found_failed")
            )
            validation_log[
                "val_success_found_failed_return_margin"
            ].append(
                val_summary.get("success_found_failed_return_margin")
            )
            validation_log["val_negative_action_gain_count"].append(
                int(val_summary.get("negative_action_gain_count", 0))
            )
            validation_log["curriculum_gate"].append(
                copy.deepcopy(curriculum._last_validation_gate)
                if not stage4_validation
                else {}
            )
            ...
            validation_log["stage4_controlled_refresh_opportunities"].append(
                stage4_controlled_opportunities
            )
            validation_log["stage4_controlled_refresh_recovery_rate"].append(
                stage4_controlled_recovery_rate
            )
            validation_log[
                "stage4_controlled_refresh_mean_recovery_time"
            ].append(stage4_controlled_recovery_time)
            ...
            if (
                ...
                env.stage3_near_prob = 0.0
                print(
                    f"  [stage4 entry] 进入持续覆盖阶段 | source={source_path} | "
                    f"phase={curriculum.stage4_phase} | "
                    f"step_mix={Stage4HorizonMixScheduler.PHASE_MIXES['early']}"
                )

        if curriculum.curriculum_failed:
            failure_path = os.path.join(model_dir, "ppo_curriculum_failed.pth")
            agent.save(failure_path, checkpoint_training_state(episode))
            print(
                "训练因课程能力门槛未达标而停止 | "
                f"reason={curriculum.curriculum_failure_reason} | checkpoint={failure_path}"
            )
            break

        if curriculum._stage4_stop_requested:
            goal_path = os.path.join(model_dir, "ppo_stage4_goal_met.pth")
            agent.save(goal_path, checkpoint_training_state(episode))
            best_model_paths["stage4_goal_met"] = goal_path
            print(
                "Stage 4最终门槛已通过，训练正常结束 | "
                f"episode={episode} | checkpoint={goal_path}"
            )
            break
    ...
    try:
        for stage in config["eval_stages"]:
            ...
            for scene_index, scene_key in enumerate(config["eval_scene_keys"]):
                ...
                env.set_stage2_spawn_mix(
                    (0.0, 0.0, 1.0)
                    if int(stage) == 2
                    else FireSearchBaselineEnvironment.STAGE2_FINAL_SPAWN_MIX
                )
                env.set_coverage_objective(
                    "fresh"
                    if full_horizon
                ...
                env.set_controlled_refresh_enabled(
                    bool(full_horizon and int(stage) == 3)
                )
                ...
                for episode_index in range(eval_episodes_per_scene):
                    ...
                    record = {
                        ...
                        "first_boundary_step": int(info.get("first_boundary_step", -1)),
                        "spawn_modes": list(info.get("spawn_modes", [])),
                        "stage1_discovered_within_deadline": bool(
                            info.get("stage1_discovered_within_deadline", False)
                        ),
                        ...
                        "stage2_contact_success": bool(
                            info.get("stage2_contact_success", False)
                        ),
                        "stage2_reacquire_success": bool(
                            info.get("stage2_reacquire_success", False)
                        ),
                        "stage2_tracking_steps": int(
                            info.get("stage2_tracking_steps", 0)
                        ),
                        ...
                        "controlled_refresh_opportunity": bool(
                            info.get("controlled_refresh_opportunity", False)
                        ),
                        "controlled_refresh_recovered": bool(
                            info.get("controlled_refresh_recovered", False)
                        ),
                        "controlled_refresh_recovery_time": info.get(
                            "controlled_refresh_recovery_time"
                        ),
                        ...
                        "negative_action_gain_count": int(
                            info.get("negative_action_gain_count", 0)
                        ),
                        "reward_breakdown": info.get("reward_breakdown") or {},
                    }
                    if full_horizon:
            ...
            if full_horizon:
                ...
                stage1_deadline_found_records = [
                    r
                    for r in stage_records
                    if r.get("stage1_discovered_within_deadline", False)
                ]
                far_spawn_records = [
                    r
                    for r in stage_records
                    if any(
                        str(mode) == "stage2_far"
                        for mode in r.get("spawn_modes", [])
                    )
                ]
                success_records = [r for r in stage_records if r["success"] > 0.5]
                found_failed_records = [
                    r
                ...
                def _mean_or_none(records: List[Dict], key: str):
                    return (
                        float(np.mean([r[key] for r in records])) if records else None
                    )

                def _fresh_mean_or_none(records: List[Dict]):
                    return (
                        float(
                            np.mean(
                                [
                                    float(r["reward_breakdown"].get("r_fresh_gain", 0.0))
                                    for r in records
                                ]
                            )
                        )
                ...
                success_return = _mean_or_none(success_records, "reward")
                found_failed_return = _mean_or_none(found_failed_records, "reward")
                results[int(stage)] = {
                    ...
                    "zero_discovery_timeout_count": int(
                        sum(r["zero_discovery_timeout"] for r in stage_records)
                    ),
                    "boundary_found_count": len(found_records),
                    "boundary_found_rate": float(
                        np.mean(
                    ...
                    "stage1_discovery_within_deadline_rate": float(
                        len(stage1_deadline_found_records) / max(len(stage_records), 1)
                    ),
                    "stage1_deadline_found_count": len(stage1_deadline_found_records),
                    "stage1_tracking_success_count": int(
                        sum(
                            bool(r["stage1_tracking_success"])
                            for r in stage1_deadline_found_records
                        )
                    ),
                    "stage1_tracking_given_found_rate": float(
                        sum(
                            bool(r["stage1_tracking_success"])
                            for r in stage1_deadline_found_records
                        )
                    ...
                    "stage2_found_count": len(found_records),
                    "stage2_contact_success_count": int(
                        sum(bool(r["stage2_contact_success"]) for r in found_records)
                    ),
                    "stage2_contact_given_found_rate": float(
                        sum(bool(r["stage2_contact_success"]) for r in found_records)
                        / max(len(found_records), 1)
                    ),
                    "stage2_reacquire_success_rate": float(
                        np.mean([r["stage2_reacquire_success"] for r in stage_records])
                    ),
                    ...
                    "median_first_boundary_step_far": (
                        float(
                            np.median(
                                [
                                    r["first_boundary_step"]
                                    for r in far_spawn_records
                                    if r["first_boundary_step"] > 0
                                ]
                            )
                        )
                        if any(
                            r["first_boundary_step"] > 0 for r in far_spawn_records
                        )
                        else None
                    ),
                    ...
                    "median_unique_boundary_cells_given_found": (
                        float(
                            np.median(
                                [
                                    r["stage1_unique_boundary_cells"]
                                    for r in found_records
                                ]
                            )
                        )
                    ...
                    "mean_return_success": success_return,
                    "mean_return_found_failed": found_failed_return,
                    "mean_return_never_found": _mean_or_none(
                        never_found_records, "reward"
                    ),
                    "success_found_failed_return_margin": (
                        success_return - found_failed_return
                        if success_return is not None
                        and found_failed_return is not None
                        else None
                    ),
                    "mean_fresh_gain_success": _fresh_mean_or_none(success_records),
                    "mean_fresh_gain_found_failed": _fresh_mean_or_none(
                        found_failed_records
                    ),
                    "mean_fresh_gain_never_found": _fresh_mean_or_none(
                        never_found_records
                    ),
                    "negative_action_gain_count": int(
                        sum(r["negative_action_gain_count"] for r in stage_records)
                    ),
                ...
                if (
                    results[int(stage)]["success_rate"]
                    > results[int(stage)]["boundary_found_rate"] + 1e-9
                ):
                    raise RuntimeError(
                        "evaluation inconsistency: success_rate exceeds boundary_found_rate"
                    )
            ...
            if full_horizon:
                ...
                if int(stage) == 1:
                    success_return = summary.get("mean_return_success")
                    failed_return = summary.get("mean_return_found_failed")
                    margin = summary.get("success_found_failed_return_margin")
                    print(
                        "  回报分离 | "
                        f"success={success_return if success_return is not None else 'N/A'} | "
                        f"found_failed={failed_return if failed_return is not None else 'N/A'} | "
                        f"margin={margin if margin is not None else 'N/A'} | "
                        f"negative_action_gain={summary.get('negative_action_gain_count', 0)}"
                    )
    finally:
