...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    def reset(self) -> Dict:
        ...
        if self.observation_profile == "cooperative_exploration" or self.communication_enabled:
    ...
    def _compute_reward(
        ...
        if self.termination_mode == "post_target_train" and self.target_reached:
            step_penalty = self.post_target_step_penalty
    ...
    def step(self, actions: List[int]) -> Tuple[Dict, List[float], bool, Dict]:
        ...
        for i, action in enumerate(actions):
            ...
            if (
                self.observation_profile == "cooperative_exploration"
        ...
        self.coverage_history.append(float(coverage))
        if self.termination_mode == "post_target_train":
            ...
            if coverage >= 0.70 and not self.post_target_milestones["0.70"]:
                milestone_reward += self.post_target_milestone_70
                self.post_target_milestones["0.70"] = True
            if coverage >= 0.80 and not self.post_target_milestones["0.80"]:
            ...
            if self.target_reached:
                hold_reward = self.post_target_hold_weight * float(
                    np.clip(
                        (coverage - self.post_target_goal) / (1.0 - self.post_target_goal),
                        -1.0,
                        1.0,
                    )
                )
        ...
        if done:
            if done_reason == "mission_complete":
                ...
                terminal_penalty = self._timeout_terminal_penalty(coverage)
