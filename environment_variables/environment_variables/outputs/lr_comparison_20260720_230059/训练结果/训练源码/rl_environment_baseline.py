...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    def __init__(
        ...
        post_target_step_penalty: float = -0.02,
        ...
        assigned_boundary_gain_weight: float = 1.0,
        role_switch_penalty: float = 0.05,
    ...
    def _boundary_coverage_gain_reward(self, new_points: int) -> float:
        ...
        reward = self.coverage_gain_weight * delta_coverage
        return float(np.clip(reward, 0.0, self.coverage_gain_clip))
    ...
    def _timeout_terminal_penalty(self, coverage: float) -> float:
        if self.curriculum_stage >= 2:
            ...
            penalty = 20.0 + 30.0 * miss_gap
            if float(coverage) <= 1e-9:
                penalty += getattr(self, "zero_coverage_timeout_extra_penalty", 25.0)
            return float(penalty)

        early_gap = max(0.0, 0.02 - float(coverage)) / 0.02
        return float(10.0 + 20.0 * early_gap)
    ...
    def _boundary_freshness_metrics(self) -> Tuple[float, float]:
        ...
        for by, bx in self.boundary_points:
            ...
            age = max(0, int(self.step_count) - newest)
    ...
    def _stage_target_reached(self, coverage: float) -> bool:
        if self.curriculum_stage == 1:
            return bool(self.stage1_tracking_success)
        target = self.stage_targets[2] if self.curriculum_stage == 2 else self.stage_targets[3]
    ...
    def _effective_post_target_step_penalty(self) -> float:
        if not hasattr(self, "post_target_step_cost_fraction"):
            return self.post_target_step_penalty
        ...
        if first_target_step is None:
            remaining_horizon = extra_steps or self.max_steps
        elif extra_steps is not None:
            remaining_horizon = extra_steps
        else:
            remaining_horizon = max(1, self.max_steps - first_target_step)
        milestone_scale = max(20.0, self.post_target_milestone_70)
        budget_cap = (
            self.post_target_step_cost_fraction * milestone_scale
            / max(float(remaining_horizon), 1.0)
        )
        return -min(abs(self.post_target_step_penalty), budget_cap)
    ...
    def step(self, actions: List[int]) -> Tuple[Dict, List[float], bool, Dict]:
        ...
        if persistent_boundary:
            ...
            team_fresh_reward = (
                self.fresh_coverage_gain_weight * self.coverage_action_gain
            )
            rewards = [
        ...
        if self.curriculum_stage == 1 and self.first_boundary_step > 0:
            ...
            if len(self.stage1_tracking_window) > 25:
            ...
            self.stage1_tracking_success = bool(
                len(self.stage1_tracking_window) >= 25
                and sum(self.stage1_tracking_window) >= 20
                and len(self.discovered_boundary) >= 12
                and tracking_progress >= 8
            )
