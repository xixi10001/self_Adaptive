...
class CurriculumManager:
    ...
    STAGE2_MIN_EPISODES = (100, 125, 150)
    STAGE2_MAX_EPISODES = (150, 200, 250)
    STAGE2_FAR_SPAWN_RATIOS = (0.50, 0.80, 1.00)
    STAGE2_FAR_RAMP_GATES = (
        {"boundary_found_rate": 0.80, "success_rate": 0.70, "zero_timeout": 0.20},
        {"boundary_found_rate": 0.85, "success_rate": 0.75, "zero_timeout": 0.15},
    )
    STAGE3_SUBSTAGES = ("3A", "3B", "3C", "3D")
    STAGE3_TARGET_MIN_EPS = (150, 180, 250, 350)
    STAGE3_TARGET_MAX_EPS = (200, 250, 350, 500)
    ...
    STAGE4_GOALS = (0.60, 0.65, 0.70)
    STAGE4_MIN_EPISODES = (100, 150, 250)
    STAGE4_MAX_EPISODES = (200, 250, 350)
    STAGE1_MIN_EPISODES = 150
    ...
    def stage2_far_spawn_ratio(self) -> float:
        if self.current_stage == 2 and self.current_substage == "2A":
            return self.STAGE2_FAR_SPAWN_RATIOS[self._stage2_far_ratio_idx]
        return 1.0
    ...
    def _minimum_episodes_reached(self) -> bool:
        if self.current_stage == 1:
            ...
            required = self.STAGE4_MIN_EPISODES[self._stage4_level_idx]
    ...
    def base_current_budget_cap(self) -> int:
        ...
        if self.current_stage == 2:
            return self.STAGE2_MAX_EPISODES[self._stage2_idx]
        if self.current_stage == 3:
            return self.STAGE3_TARGET_MAX_EPS[self._s3_target_idx]
        return self.STAGE4_MAX_EPISODES[self._stage4_level_idx]

    @property
    def current_budget_cap(self) -> int:
        return int(self.base_current_budget_cap + self._transferable_budget)

    def _maybe_advance_stage2_far_ratio(self, summary: Dict) -> bool:
        if (
            self.current_stage != 2
            or self.current_substage != "2A"
            or self._stage2_far_ratio_idx >= len(self.STAGE2_FAR_SPAWN_RATIOS) - 1
        ):
            return False
        gate = self.STAGE2_FAR_RAMP_GATES[self._stage2_far_ratio_idx]
        ...
        passed = bool(
            found >= gate["boundary_found_rate"]
            and success >= gate["success_rate"]
            and zero_timeout <= gate["zero_timeout"]
        )
        if not passed:
            return False
        old_ratio = self.stage2_far_spawn_ratio
        self._stage2_far_ratio_idx += 1
        print(
            "\n  [2A远起点比例] "
            f"{old_ratio:.0%} -> {self.stage2_far_spawn_ratio:.0%} | "
            f"boundary_found={found:.1%} | success={success:.1%} | "
            f"zero_discovery_timeout={zero_timeout:.1%}"
        )
        return True
    ...
    def validation_gate_status(self, summary: Dict) -> Dict[str, Dict[str, object]]:
        ...
        contact_value = None if median_contact is None else float(median_contact)
        if self.current_stage == 1:
            checks = {
                "boundary_found": (found, 0.90, ">="),
                "stable_tracking": (
                    float(summary.get("stable_tracking_success_rate", 0.0)),
                    0.70,
                    ">=",
                ),
                "zero_discovery_timeout": (zero_timeout, 0.10, "<="),
                "median_first_boundary_step": (contact_value, 100.0, "<="),
            ...
            checks = {
                "boundary_found": (found, 0.85, ">="),
                "stable_contact_or_reacquire": (success, 0.75, ">="),
                "zero_discovery_timeout": (zero_timeout, 0.10, "<="),
                "median_first_boundary_step": (contact_value, 150.0, "<="),
            }
        else:
            required_success = 0.70 if self._s3_target_idx < 2 else 0.65
            checks = {
                "boundary_found": (found, 0.85, ">="),
                "target_success": (success, required_success, ">="),
                "zero_discovery_timeout": (zero_timeout, 0.10, "<="),
            }
        return {
            name: {
    ...
    def _validation_passed(self, summary: Dict) -> bool:
        status = self.validation_gate_status(summary)
        self._last_validation_gate = status
        return bool(status and all(check["passed"] for check in status.values()))

    def _advance_validated_substage(self) -> str:
        old_substage = self.current_substage
        unused_budget = max(
            0,
            self.current_budget_cap - self.substage_episodes[old_substage],
        )
        self._transferable_budget = int(unused_budget)
    ...
    def _print_validation_gate(self) -> None:
        print(f"  [课程门槛] substage={self.current_substage}")
        for name, check in self._last_validation_gate.items():
            actual_raw = check["actual"]
            ...
            if name in {
                ...
                "stable_tracking",
                "zero_discovery_timeout",
                "stable_contact_or_reacquire",
                "target_success",
            }:
                actual_text = "None" if actual is None else f"{actual * 100:.1f}%"
                threshold_text = f"{threshold * 100:.1f}%"
            else:
                actual_text = "None" if actual is None else f"{actual:.1f}"
        ...
        print(
            f"    validation_window={sum(self._validation_history)}/"
            f"{len(self._validation_history)} | episodes="
            f"{self.substage_episodes[self.current_substage]}/{self.current_budget_cap} | "
            f"transferable_budget={self._transferable_budget} | "
            f"stage2_far_ratio={self.stage2_far_spawn_ratio:.0%}"
        )

    def update_validation(self, summary: Dict) -> bool:
        if self.current_stage == 4:
            return False
        passed = self._validation_passed(summary)
        self._validation_history.append(passed)
        if self._maybe_advance_stage2_far_ratio(summary):
            self._print_validation_gate()
            return False
        if not self._minimum_episodes_reached():
            self._print_validation_gate()
            return False
        self._print_validation_gate()
        if (
            ...
            if self.substage_episodes[self.current_substage] >= self.current_budget_cap:
                self.curriculum_failed = True
                self.curriculum_failure_reason = (
                    f"{self.current_substage} exhausted its {self.current_budget_cap}-episode budget "
                    "without passing validation"
                )
                print(f"\n  [课程失败] {self.curriculum_failure_reason}")
            return False

        old_substage = self._advance_validated_substage()
        next_label = (
    ...
    def state_dict(self) -> Dict:
        return {
            ...
            "stage2_far_ratio_index": self._stage2_far_ratio_idx,
            "stage2_far_spawn_ratio": self.stage2_far_spawn_ratio,
            "transferable_budget": self._transferable_budget,
...
def _build_experiment_metadata(
    ...
    return {
        ...
        "curriculum_budget": {
            ...
            "stage2_max": list(CurriculumManager.STAGE2_MAX_EPISODES),
            "stage2_far_spawn_ratios": list(
                CurriculumManager.STAGE2_FAR_SPAWN_RATIOS
            ),
            "stage3_max": list(CurriculumManager.STAGE3_TARGET_MAX_EPS),
            "stage4_max": list(CurriculumManager.STAGE4_MAX_EPISODES),
            "stage4_reserved": CurriculumManager.STAGE4_MIN_REMAINING_EPISODES,
            "unused_budget_transfer": True,
        },
        "stage1_tracking_gate": {
            ...
            "minimum_post_contact_progress": FireSearchBaselineEnvironment.STAGE1_MIN_POST_CONTACT_PROGRESS,
        },
        "stage2_contact_gate": {
