...
class FireSearchBaselineEnvironment(gym.Env):
    ...
    OBSERVATION_PROFILE_DIMS = {
        "baseline": {"local_obs_dim": 19, "global_state_dim": 19},
        "static_terrain": {"local_obs_dim": 26, "global_state_dim": 19},
        "dynamic_front": {"local_obs_dim": 25, "global_state_dim": 19},
        "risk_aware": {"local_obs_dim": 22, "global_state_dim": 19},
    }
    REWARD_PROFILES = {
        "boundary_coverage",
        "front_detection",
    ...
    def _get_observation(self) -> Dict:
        ...
        for i in range(self.num_drones):
            ...
            to_fire_y = float((self.fire_centroid[0] - pos[0]) / map_norm)
            to_fire_x = float((self.fire_centroid[1] - pos[1]) / map_norm)
