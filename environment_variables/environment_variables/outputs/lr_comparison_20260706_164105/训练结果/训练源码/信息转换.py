...
class FireSceneData:
    ...
    def _compute_thermal_field(self):
        ...
        if not hasattr(self, "_thermal_field_cache"):
            self._thermal_field_cache = {}
        ...
        small_thermal = self._thermal_field_cache.get(cache_key)

        if small_thermal is None:
            ...
            current_fire = np.zeros_like(intensity_map, dtype=np.float32)
            current_fire[fire_mask] = intensity_map[fire_mask]
            small_fire = cv2.resize(
                current_fire,
                (0, 0),
                fx=0.25,
                fy=0.25,
            ...
            small_thermal = gaussian_filter(
                small_fire,
                sigma=15,
                truncate=4.0,
            )
            self._thermal_field_cache[cache_key] = small_thermal

        height, width = fire_mask.shape
        thermal_field = cv2.resize(
            small_thermal,
            (width, height),
            interpolation=cv2.INTER_LINEAR,
        )
        self.thermal_field = np.clip(thermal_field * 800.0, 0, 100)
    ...
    def get_normalized_features(
        ...
        if getattr(self, "thermal_field", None) is not None:
            features["thermal_norm"] = float(
                self._clip01(np.tanh(self.get_thermal_value(row, col) / 50.0))
            )
        return features
